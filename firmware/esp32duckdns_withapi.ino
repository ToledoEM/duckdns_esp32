/*
ESP32 DuckDNS client v1.2 (Adapted from ESP8266 version by Davide Gironi)
*/

//main ESP32 Arduino library
#include <WiFi.h>
//DNS server library
#include <DNSServer.h>
//WebServer library
...
#define DEBUGMODE DEBUGMODE_BOTH // Changed to BOTH for easier debugging
...
//define the ddns status led (GPIO2 is often the built-in LED on ESP32 boards)
...
//ddns update internal on error (minutes)
...
const char HTTP_HEADTITLE[] PROGMEM = "<title>ESP32 Duck DNS Client</title>";
...
const char HTTP_CONTENTSTART[] PROGMEM = "<div class='b'><h1>ESP32<br/>DuckDNS client</h1><div class='m'><a href='/'>Home</a> | <a href='settings' class='s'>Settings</a></div><div class='p'>";
...
void pageApiStatus() {
  ...
  // Create JSON string
  String json = "{\"last_update_status\":\"" + lastupdatestatus + "\",\"last_update_ago\":" + String(lastupdatetime / 1000) + ",\"next_update_in\":" + String(nextupdatetime / 1000) + "}";
...
void wifiManagerSetStatus(int status) {
  if(status == WIFIMANAGERSETSTATUS_STARTAP) {
    ...
    wifiManagerStatusLedTicker.detach();
    digitalWrite(WIFIMANAGERSETSTATUS_LED, HIGH); // HIGH is OFF for many built-in LEDs
#endif
...
int ddnsUpdate() {
  ...
  if(updated) {
    ...
    digitalWrite(DDNS_STATUSLED, HIGH); // LED Off
#if defined SERIAL_ENABLED
...
void setup() {
  ...
  digitalWrite(WIFIMANAGERSETSTATUS_LED, HIGH); // Off
#endif

  //set ddns status led
  pinMode(DDNS_STATUSLED, OUTPUT);
  digitalWrite(DDNS_STATUSLED, HIGH); // Off
...
void loop() {
  ...
  if (millis() > ddns_updatetimer) {
    ...
    if(ddnsUpdate()) {
      ...
      ddns_updatetimer = millis() + (unsigned long)DDNS_ONERRORINTERVAL * 60 * 1000;
    }
  }
}
